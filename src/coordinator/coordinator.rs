            ...
            cbc::select! {
                ...
                recv(self.net_data_recv_rx) -> package => {
                   match package {
                        ...
                        Err(e) => {
                            error!("Error extracting network package in coordinator: {:?}\r\n", e);
                            std::process::exit(1);
                        }
                    }
                },
                ...
                recv(self.net_peer_update_rx) -> peer => {
                    match peer {
                        ...
                        Err(e) => {
                            error!("Error extracting peer update package in coordinator: {:?}\r\n", e);
                            std::process::exit(1);
                        }
                    }
                },
                ...
                recv(self.hw_request_rx) -> request => {
                    match request {
                        ...
                        Err(e) => {
                            error!("Error extracting button package in coordinator: {:?}\r\n", e);
                            std::process::exit(1);
                        }
                    }
                },
                ...
                recv(self.fsm_order_complete_rx) -> completed_order => {
                    match completed_order {
                        ...
                        Err(e) => {
                            error!("Error extracting completed order from fsm in coordinator: {:?}\r\n", e);
                            std::process::exit(1);
                        }
                    }
                }
